...
# 화면 타이틀 설정
pygame.display.set_caption("SY Pang!") # 게임 이름
...
# 캐릭터 이동 방향
character_to_x_LEFT = 0 # 오류 수정 위해 LEFT 와 RIGHT 로 나눠서 변수 생성
    ...
    for event in pygame.event.get(): 
        ...
        if event.type == pygame.KEYDOWN :
            if event.key == pygame.K_LEFT: # 캐릭터를 왼쪽으로
                character_to_x_LEFT -= character_speed # 수정
            elif event.key == pygame.K_RIGHT : # 캐릭터를 오른쪽으로
                character_to_x_RIGHT += character_speed  # 수정
            elif event.key == pygame.K_SPACE : # 무기 발사
                weapon_x_pos = character_x_pos + (character_width / 2) - (weapon_width / 2)
                weapon_y_pos = character_y_pos
                weapons.append([weapon_x_pos, weapon_y_pos])

        # 수정 3 : 키에서 손을 뗄 때 LEFT, RIGHT 를 각각 처리
